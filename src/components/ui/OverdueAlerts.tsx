...
export function OverdueAlerts() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Seznam zapadlih računov</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Številka</TableHead><TableHead>Kupec</TableHead><TableHead className="text-right">Znesek</TableHead>
                <TableHead>Datum zapadlosti</TableHead><TableHead>Dni zamude</TableHead><TableHead></TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {overdue.map(inv => {
                ...
                return (
                  <TableRow key={inv.id}>
                    ...
                    <TableCell><div>{inv.customerName}</div><div className="text-xs text-gray-500">{inv.customerTaxId}</div></TableCell>
                    <TableCell className="text-right font-medium">{formatCurrency(inv.totalGross)}</TableCell>
                    <TableCell>{formatDate(inv.dueDate)}</TableCell>
                    <TableCell><Badge className="bg-blue-100 text-blue-800"><Clock className="w-3 h-3 mr-1" />{daysLate} dni</Badge></TableCell>
                    <TableCell>
                      <div className="flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => setSelectedInvoiceId(inv.id)}><Eye className="w-4 h-4" /></Button>
                        <Button size="sm" variant="secondary" onClick={() => handleSendReminder(inv)}><Mail className="w-4 h-4 mr-1" /> Opomin</Button>
                        <Button size="sm" onClick={() => handleMarkAsPaid(inv.id)}><CheckCircle className="w-4 h-4 mr-1" /> Plačano</Button>
                      </div>
                    </TableCell>
                  </TableRow>
                )
