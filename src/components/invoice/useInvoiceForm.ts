...
const calculateTotals = (items: InvoiceItem[], discountPercent: number) => {
  ...
  return { totalNetBeforeDiscount, totalItemDiscounts, totalNetAfterItemDiscounts, invoiceDiscountAmount, discountPercent, finalNetBase, vatBreakdown, totalVat, totalGross }
}
...
export function useInvoiceForm(editingInvoice: Invoice | null | undefined, customers: Customer[]) {
  ...
  useEffect(() => {
    if (serviceDateFrom && serviceDateTo && serviceDateTo < serviceDateFrom) setDateError('Datum "do" ne more biti pred datumom "od"')
    else setDateError('')
  }, [serviceDateFrom, serviceDateTo])

  useEffect(() => {
    if (discountPercent < 0 || discountPercent > 100) setDiscountError('Popust mora biti med 0 in 100')
  ...
  return {
    ...
    serviceDateTo, setServiceDateTo,
    dueDate, setDueDate,
    note, setNote,
    dateError,
    totals,
