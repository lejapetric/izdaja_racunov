...
function App() {
  ...
  const renderView = () => {
    switch (activeView) {
      case 'new-invoice': return <NewInvoice editingInvoice={editingInvoice} clearEditing={() => setEditingInvoice(null)} />
      case 'archive': return <InvoiceArchive onEditInvoice={(inv) => { setEditingInvoice(inv); setActiveView('new-invoice') }} />
      case 'overdue': return <OverdueAlerts />
      case 'estimates': return <Estimates setActiveView={setActiveView} />
