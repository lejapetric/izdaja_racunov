...
export function Reports() {
  ...
  // Filtri - isti kot v InvoiceFilters
  ...
  const stats = (() => {
    ...
    filteredInvoices.forEach(inv => { 
      if (!byCustomer[inv.customerId]) byCustomer[inv.customerId] = { name: inv.customerName, count: 0, amount: 0 }
          ...
          <CardContent>
            {reportType === 'list' ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <Card className="bg-purple-50">
                    <CardContent className="pt-6">
                      ...
                      <p className="text-2xl font-bold text-purple-800">{stats.count}</p>
                    </CardContent>
                  </Card>
                </div>
                
                <Card>
                  <CardHeader><CardTitle>Po statusih</CardTitle></CardHeader>
                  <CardContent>
                    {Object.entries(stats.byStatus).map(([status, data]) => (
                      <div key={status} className="flex justify-between p-2 border-b">
                        <span>{statusLabels[status]}</span>
                        <span>{data.count} računov • {formatCurrency(data.amount)}</span>
                      </div>
                    ))}
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader><CardTitle>Po kupcih (top 10)</CardTitle></CardHeader>
                  <CardContent>
                    {Object.entries(stats.byCustomer)
                      .sort((a, b) => b[1].amount - a[1].amount)
                      .slice(0, 10)
                      .map(([_, customer]) => (
                        <div key={customer.name} className="flex justify-between p-2 border-b">
                          <span>{customer.name}</span>
                          <span>{customer.count} računov • {formatCurrency(customer.amount)}</span>
                        </div>
                      ))}
                  </CardContent>
                </Card>
